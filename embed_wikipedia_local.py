#!/usr/bin/env python3
"""Embed the 41.97M wikipedia-en 120-token chunks with all-MiniLM-L6-v2 on the 5090.
...
pattern of /data/raw/common-corpus/embed_probes.py, scaled to the full corpus:
  ...
  input : /data/chunks/wikipedia-en-chunked-120/train/data-000NN-of-00041.parquet
  output: /data/embeddings/wikipedia-en-chunked-120-all-MiniLM-L6-v2/train/
            data-000NN-of-00041.npy   # (rows, 384) float16, row-parallel with
...
Run (only when the GPU is free — check nvidia-smi first):
  cd ~/code/latent-basemap && systemd-run --user --unit=wiki-embed \
    .venv/bin/python /home/enjalot/code/latent-data-modal/embed_wikipedia_local.py
...
CHUNKS = "/data/chunks/wikipedia-en-chunked-120/train"
OUT = "/data/embeddings/wikipedia-en-chunked-120-all-MiniLM-L6-v2/train"
...
def main() -> int:
    ...
    shards = sorted(glob.glob(f"{CHUNKS}/data-*-of-00041.parquet"))
    if SHARD_LIMIT:
    ...
    if SMOKE:
        ...
        print(f"SMOKE: 10k rows in {dt:.1f}s = {10_000/dt:,.0f} chunks/s "
              f"-> 42M in ~{41_970_000/(10_000/dt)/3600:.1f} h; shape {vecs.shape}")
        return 0
    ...
    for path in shards:
        ...
        rate = len(texts) / dt
        done_frac = total_rows / 41_970_000
